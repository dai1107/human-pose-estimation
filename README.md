...
本机 30 帧网页协议探针（640 px、JPEG 0.65）测得往返延迟 P50 `18.7 ms`、P95 `35.5 ms`，此前轮询链路约为 `58 ms`；服务器处理 P50 `17.2 ms`，其中姿态推理 P50 `14.4 ms`，姿态年龄 P50 `16.0 ms`。固定箭步蹲示例 133 帧的无播放节流计算基准约 `1.1 s`，运行时推理 `0.0 ms`，原推理链路约 `15.7 s`；页面现在始终按源视频 30 FPS 播放，因此实际展示时长保持原始的约 `4.43 s`。这些数字是当前测试机基线，不是对所有硬件与网络的承诺；示例仍需视频解码、规则计算、绘图和 JPEG 编码。

当前完整自动化回归为 `487 passed`；另已完成 133 帧箭步蹲示例实链路复验，显示出的角度来源全部为 `3d`。
...
One Euro 当前正式配置默认使用 `responsive` 档；`stable` 更强调稳定，`balanced` 在稳定与响应之间折中。三档参数、节点组倍率、world landmarks 倍率和长间隔阈值都在 `configs/product_pose.yaml` 的 `realtime_smoothing` 段经过严格校验。桌面可用 `--smoothing-profile stable|balanced|responsive` 临时覆盖档位。image landmarks 与 world landmarks 使用完全独立的滤波状态；HYROX 动作阶段和原始二维规则仍消费 image landmarks，world landmarks 只进入三维显示、质量审计和 Assist 置信度层。
