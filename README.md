...
- **固定示例零模型推理**：8 个内置示例都带有视频、姿态模型、手部模型和必要 YOLO 模型的指纹校验缓存。页面显示“预计算示例结果”，姿态推理时间为 `0.0 ms`；箭步蹲的 YOLO 人物锁定仅发生在离线缓存生成阶段。缓存缺失、损坏或指纹不一致时才安全回退真实推理。
- **实时摄像头低延迟**：桌面和网页均采用最新帧优先、单任务在途、忙时丢弃旧帧、帧序号/采集时间关联和过期姿态拒绝。网页 WebSocket 结果由事件驱动线程在就绪后立即推送，不再等待轮询；手指跟踪默认关闭以避免额外手部推理。
- **自适应平滑**：One Euro 使用姿态对应帧的真实采集时间，image/world landmarks 状态分离，腕踝等快速节点和肩髋等稳定节点使用不同倍率，长于 250 ms 的观测间隔会重置轨迹。当前正式配置使用 `responsive`，也可显式选择 `stable` 或 `balanced`。
- **3D Assist**：MediaPipe world landmarks 并行提供膝、髋、肘、肩三维角度和可靠性审计。可靠且与二维一致的证据只增强相关规则置信度，严重冲突会降为 `UNSURE`，不可用时完整回退二维；动作阶段、二维阈值、触地、地板、起落、腕部时序、补步和距离规则没有被三维替换。
...
本机 30 帧网页协议探针（640 px、JPEG 0.65）测得往返延迟 P50 `18.7 ms`、P95 `35.5 ms`，此前轮询链路约为 `58 ms`；服务器处理 P50 `17.2 ms`，其中姿态推理 P50 `14.4 ms`，姿态年龄 P50 `16.0 ms`。固定箭步蹲示例最终接口复验 133 帧约 `1.1 s`，运行时推理 `0.0 ms`，原链路约 `15.7 s`。这些数字是当前测试机基线，不是对所有硬件与网络的承诺；示例仍需视频解码、规则计算、绘图和 JPEG 编码，因此总耗时不会为零。

3D Assist 完成时全量回归基线为 `479 passed`；示例缓存、网页接口和实时链路的最终聚焦回归为 `126 passed`。
...
1. **识别并平滑关键点**：姿态后端输出肩、肘、腕、髋、膝、踝、脚跟和脚尖等关键点及置信度；实时流经过自适应 One Euro 平滑。滤波时间差来自姿态对应帧的真实采集时间，不使用显示刷新、callback 到达时间或固定 30 FPS；腕、踝、脚跟和脚尖采用更高响应参数，肩髋采用更稳定参数，超过 250 ms 的观测间隔会重置轨迹。
...
One Euro 当前正式配置默认使用 `responsive` 档；`stable` 更强调稳定，`balanced` 在稳定与响应之间折中。三档参数、节点组倍率、world landmarks 倍率和长间隔阈值都在 `configs/product_pose.yaml` 的 `realtime_smoothing` 段经过严格校验。桌面可用 `--smoothing-profile stable|balanced|responsive` 临时覆盖档位。image landmarks 与 world landmarks 使用完全独立的滤波状态；当前 HYROX 状态机仍只消费既有 image landmarks，不会因保存和平滑 world landmarks 改变计数。

MediaPipe world landmarks 当前处于严格的 `assist` 模式：系统并行计算膝、髋、肘、肩的 3D 角度并执行 visibility/presence、骨段、z 跳变、角速度、身份交换、姿态年龄和观测间隔门控。所有 `selected_angle` 仍取二维值，动作阶段和规则 PASS/FAIL 仍使用原有二维阈值；可靠且与二维一致的 3D 只提高映射角度规则的置信度，原本 `UNSURE` 的规则不会被越级改为 `VALID`，严重 2D/3D 冲突会将相关候选降为 `UNSURE`，3D 缺失或不可靠时则保持完全二维回退。触地、地板、髋膝图像高度、腕部位置与同步、起跳、落地、补步和距离规则不使用 3D。网页和桌面报告继续保存 2D/3D 差值、Assist 状态、冲突比例与失败原因；这些统计依赖实际视频，程序不会伪造硬件或动作结论。
...
实时摄像头和上传视频只使用 `MediaPipe Pose`；`auto` 仅作为旧请求兼容值并直接映射到 MediaPipe。8 个固定示例读取随版本生成的姿态与手部关键点缓存，模型栏显示“预计算示例结果”，播放时不再加载姿态或手部模型。负重箭步蹲缓存由离线 YOLO 人物锁定与 MediaPipe 关键点融合生成，用于避开样例中的背景人物；该 YOLO 路径不会进入摄像头或上传视频会话。视频或相关模型文件变化时缓存会自动失效并回退真实推理。
