...
GPU 环境应至少包含 `CUDAExecutionProvider`；只有 `CPUExecutionProvider` 时仍可运行，但 RTMW-X/L 的处理速度会明显降低。正式网页摄像头、上传视频和默认桌面链路只使用 MediaPipe Pose，YOLO/RTMW 仅供显式实验、离线比较、缓存生成和研究消融；安装 RTMW 不会让产品网页出现模型切换项。实验运行时模型仍在服务器执行，手机摄像头、编码性能与网络质量会影响端到端体验。
