...
COMMAND_JSON_ENV = "POSE_WHAM_COMMAND_JSON"
COMMAND_ENV = "POSE_WHAM_COMMAND"


def command_template_from_environment() -> list[str]:
    """Read an argv template without invoking a shell.

    JSON-array form is preferred, for example:
    ``["wsl", "python", "run_wham.py", "--video", "{video_path}",
    "--output", "{output_json}"]``.
    """
