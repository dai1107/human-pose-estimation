...
> 当前实现说明（2026-08-11）：本文主要描述评分与规则分层设计。实时姿态优化任务第 1–12 轮已完成第一版工程实现，但没有改变本文定义的 HYROX 业务语义。当前正式判定仍以 2D 分析流、动作状态机、必需规则和 `VALID / NO_REP / UNSURE` 可观测性门控为准；统一 3D/canonical 指标、地板估计与接触置信度只作辅助或验证。第 12 轮严格角度非回归未通过，不允许据此直接迁移阈值或把 3D 提升为正式裁决。运行现状以 [项目 README](README.md)、[使用说明](使用说明.md) 和 [实时优化任务清单](HYROX%20实时姿态系统优化任务清单.md) 为准。
...
>
> 当前实现状态：本文是四层评分的设计方案，不代表所有评分层已经落地。
